...
const getTodos = (resource, callback) => {
  ...
  request.addEventListener(`readystatechange`, () => {
    // 'readystatechange' goes through the 4 phases of the http request.
    if (request.readyState === 4 && request.status === 200) {
      // The readyState returns the request's phase as a number. We only can work with 4 because at 0-3 the request is not ready. 404 error is not a good status, 200 is.
  ...
  request.open(
    ...
    /* - Absolute or Relative URL `scripts/todos.json` */ resource,
  ); // 'GET' or 'POST' is the data request type and the URL is the source to get data from.
  request.send();
};
...
const getSomething = () => {
  return new Promise((resolve, reject) => {
    // Promise is a built-in API. Parameter resolve works when the request successful, reject works when the request has error.
    // fetch something
...
const getPromiseTodos = (resource) => {
  return new Promise((resolve, reject) => {
    ...
    request.open(
      `GET`,
      /* `https://jsonplaceholder.typicode.com/todos/` - Absolute or Relative URL `scripts/todos.json` */ resource,
    ); // 'GET' or 'POST' is the data request type and the URL is the source to get data from.
    request.send();
  });
};

getPromiseTodos(`https://jsonplaceholder.typicode.com/todos/`)
  .then((data) => {
...
fetch(`https://jsonplaceholder.typicode.com/todos/`)
  .then((response) => {
    // This returns a promise which is only rejected at network errors.
